    ...
    public function handle()
    {
        ...
        foreach($data_parent as $single_data){
            ...
            if($calc_level == 6)
            {
                ...
                $checkItems = VoluntaryQuotation::select('item_code','voluntary_quotation.id')->leftJoin('voluntary_quotation_sku_listing as vqsl','vqsl.vq_id','=','voluntary_quotation.id')->where('voluntary_quotation.id',$single_data['id'])->where('vqsl.discount_percent','>=',30)->exists();
                if($checkItems == true)
                {
                    $checkExceptionsItems = VoluntaryQuotation::select('item_code','div_id')->leftJoin('voluntary_quotation_sku_listing as vqsl','vqsl.vq_id','=','voluntary_quotation.id')->where('voluntary_quotation.id',$single_data['id'])->where('vqsl.discount_percent','>=',30)->get();
        ...
        if($vq_flag==1){
            if($mail_level<=6){
                ...
                $data['email_cc'] = array();
                array_push($data['email_cc'],'thomas.edakalathoor@sunpharma.com');
                array_push($data['email_cc'],'Achyut.Redkar@Sunpharma.Com');
        ...
        // CEO auto approval of vq idap-33
        $vq_flag=0;
        $reinit_vq_flag=0;
        //Parent vq Auto Approval Process
        $approval_period = ApprovalPeriod::get();
        $data_parent = VoluntaryQuotation::where('year',$year)->where('current_level',8)->where('parent_vq_id',0)->where('is_deleted', 0)->get();
        foreach($data_parent as $single_data){
            // $vq_date = strtotime($single_data['created_at']);
            // $datediff = $current_date - $vq_date;
            // $diff = round($datediff / (60 * 60 * 24));
            ...
            $vq_date = $single_data['created_at'] ? strtotime($single_data['created_at']) : strtotime(date("Y-m-d H:i:s", strtotime("-2 months"))) ;
            $datediff = $current_date - $vq_date;
            $diff = ($datediff / (60 * 60 * 24));
            $calc_level = $single_data['current_level'];
        ...
        $data_reinit = VoluntaryQuotation::where('year',$year)->where('current_level',8)->where('parent_vq_id','!=',0)->where('is_deleted', 0)->get();
        foreach($data_reinit as $single_data){
            $vq_date = $single_data['created_at'] ? strtotime($single_data['created_at']) : strtotime(date("Y-m-d H:i:s", strtotime("-2 months"))) ;
            $datediff = $current_date - $vq_date;
            $diff = ($datediff / (60 * 60 * 24));
            ...
            if($diff > $approval_period->where('type', '=', 'reinitvq_normal')->where('level',$calc_level)->first()['days']){
                $calc_level--;
            }
            if($calc_level != $single_data['current_level']){
                $reinit_vq_flag=1;
