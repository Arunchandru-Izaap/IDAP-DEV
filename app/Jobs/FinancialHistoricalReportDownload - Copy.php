...
//use Session;
class FinancialHistoricalReportDownload implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($emp_code,$emp_type,$emp_level,$div_id, $financialYear)
    {
        //
    ...
    $zip_file = 'public/financialreport'.$this->emp_code. '-'.$year.'.zip'; // Name of our archive to download
    
    $zip = new \ZipArchive();
    $zip->open($zip_file, \ZipArchive::CREATE | \ZipArchive::OVERWRITE);
    // Logic for HO user
    if($this->emp_type == 'initiator'){
    ...
    function regionGenerator($year,$zip,$region) {
        ...
        $reportNameRow = WriterEntityFactory::createRowFromArray(['IDAP - Financial Report'], $headerStyle);
        ...
        $headerRow = WriterEntityFactory::createRowFromArray(
            [
                ...
                'Type', 'Div Name', 'Pack', 'Discount Rate', 'Rev No', 'Year'
            ],
            $headerStyle
        );
            ...
            $original_list->chunk(10000, function($rows) use ($writer) {
                foreach ($rows as $row) {
                    $dataRow = WriterEntityFactory::createRowFromArray([
                        ...
                        $row->discount_rate, $row->rev_no, $row->year
                    ]);
