...
class ApproveVq implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($vq_id, $jwt, $modeofscreen, $skuIdArr=null, $changePayModeData = null)
    {
        //
        $this->vq_id = $vq_id;
        $this->jwt = $jwt;
    ...
    public function handle()
    {
        ...
        if($this->skuIdArr == null){
            ...
            try {
                ...
                foreach ($send_json['sku'] as $json_data) {
                    ...
                    elseif($this->modeofscreen == 'Paymode'):
                        if($send_json['revision_number'] !=0):
                            $discountMode = $json_data['payment_mode'];
                            $updateMode = ($discountMode === 'CN') ? ['CN', 'DM'] : ['DM', 'CN'];
                            if($discountMode == 'CN'):
                                IdapDiscTran::where('INST_ID', $send_json['institute_code'])
                ...
                if (count($send_json['sku']) === count($check_inst_id)) {
                    ...
                    try {
                        ...
                        if($jsondata->status == 1):
                            ...
                            $updation = VoluntaryQuotation::where('id',$this->vq_id)->where('is_deleted', 0)->update(['vq_status'=>1]);//added on 21032025 for updating the vq status earlier in vqlistingcontroller
                            print_r("API response done");
                            print('-');
                            print_r(date('H:i:s'));
                            print('-');
                        else:
                            print_r("API response False");
                            ...
                            try{
                                ...
                                Log::error("Api mail failed: " . $exception->getMessage());
                            }
                ...
                $maildata['Error'] = (isset($send_json['sku']))? $e->getMessage() : 'SKU items not found';
                $vq_listing_controller->activityTracker($vq->id, '1', json_encode($maildata), 'Idap_disc_tran_insert_failed');
                ...
                try{
                    ...
                    $this->serverstatuscode = "0";
                    $this->serverstatusdes = $exception->getMessage();
                    Log::error("Database insert mail failed: " . $exception->getMessage());
                }
                $check_quotation_success_status = false;
            }
        }else{
                ...
                try {
                    ...
                    foreach ($send_json['sku'] as $json_data) {
                        ...
                        if($this->modeofscreen == 'EditPaymode'): // 0th revision not getting and update in idap trans table
                            $discountMode = $json_data['payment_mode'];
                            // print_r(" ");
                            // print_r($json_data['payment_mode']);
                            // print_r(" ");
                            $updateMode = ($discountMode === 'CN') ? ['CN', 'DM'] : ['DM', 'CN'];
                            if($discountMode == 'CN'):
                                IdapDiscTran::where('INST_ID', $send_json['institute_code'])
                    ...
                    if (count($send_json['sku']) === count($check_inst_id)) {
                        ...
                        try {
                            ...
                            if($jsondata->status == 1):
                                ...
                                //code to update Vq status
                                $updation = VoluntaryQuotation::where('id',$this->vq_id)->where('is_deleted', 0)->update(['vq_status'=>1]);
                                print_r("API response Done");
                                print('-');
                                print_r(date('H:i:s'));
                                print('-');
        ...
        // print_r($check_quotation_success_status);
        // if($check_quotation_success_status == true):
        //     print_r("changePayModeData");
        //     //code to update paymode and net discount percent starts
        //     if ($this->changePayModeData != null) {
        //         $changePayModeData1 = $this->changePayModeData;
        //         foreach($changePayModeData1 as $data){
