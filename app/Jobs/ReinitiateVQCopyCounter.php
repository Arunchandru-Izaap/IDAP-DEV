...
class ReinitiateVQCopyCounter implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach($toCounters as $tocounter)
        {
            ...
            $fromInstitution = VoluntaryQuotation::select('institution_id','hospital_name')->where('institution_id', $this->fromcounter)->where('year', $year)->where('parent_vq_id',0)->where('is_deleted', 0)->first();
            $InstitutionActivity = $fromInstitution ? 'from '.$fromInstitution->hospital_name . '-' . $fromInstitution->institution_id . ' to '.$toInstitution->hospital_name. '-' .$toInstitution->institution_id : '';
            $vq_listing_controller->activityTracker($inst->id,$this->emp_code,"VQ Re-Initiated (Copy counter ".$InstitutionActivity.") by ".$this->name.'/'.$this->division_name, 'reinitiate');
