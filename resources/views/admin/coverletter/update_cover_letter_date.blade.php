...
<div class="container-fluid">
    ...
    <div class="col-md-12 d-flex justify-content-end" style="margin-bottom: 12px;margin-top: 10px;">
        <button class="btn btn-primary   mr-2" id="view_log" btn-fn="view_log">View Log</button>
    </div>
    <div class="col-md-12">
        <div class="card">
            <form class="form-horizontal" method="post" action="{{route('update_coverletter_date')}}">
                @csrf
                <div class="card-body">
                    <h4 class="card-title">Update Cover Letter date Info</h4>
                    
                    <div class="form-group row">
                        <label for="level" class="col-sm-3 text-end control-label col-form-label">Institutions</label>
                        <div class="col-sm-4">
                    ...
                    <div class="form-group row">
                      <label for="fname" class="col-sm-3 text-end control-label col-form-label">Revision No (Optional)</label>
                      <div class="col-sm-4">
                    ...
                    <div class="form-group row date_section">
                      <label for="lname" class="col-sm-3 text-end control-label col-form-label">Cover Letter Date</label>
                      <div class="col-sm-4">
                          <input type="date" id="coverletter_date" name="coverletter_date" min="<?php echo date('Y-m-d'); ?>" class="form-control">
                      </div>
                    </div>
                </div>
...
<script type="text/javascript">
  ...
  $("#institution_id").on('change', function(){
    ...
    var settings = {
        "url": "/admin/GetInstutitionRevisions",
        "method": "POST",
        "timeout": 0,
        "headers": {
    ...
    $.ajax(settings).done(function (response) {
        if(response.success == true){
            var option_html = '<option value="">Select Revision No</option>';
  ...
  $('#view_log').on('click', function(){
    ...
    $("#zero_config_view_log").DataTable({
      ...
      "pageLength": 50,
      ajax: {
        url: '/admin/cover_letter_date_updated_view_logs_ajax',
        type: 'GET',
        data: function (d) {
