...
<div id="page-content-wrapper">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        ...
                        @endphp
                        <input type='hidden' id="check_stockist_id_arr" value="<?php echo $data['check_stockist_id_arr'];?>">
                        <div class="col pd-20">
...
<script>
    ...
    function singlePayModeChangeHandler(e, ptr, inputDiscRate){
        ...
        console.log(netDiscountRateToStockist,'netDiscountRateToStockist');
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '#save_changes_btn', function(){
            ...
            if(confirmSaveChanges){
                ...
                const CheckstockistArr = $("#check_stockist_id_arr").val();
                    ...
                    for(j = 0; j < dbDataArr.length; j++){
                        ...
                        if(Number(rowData[i]['id']) == dbDataArr[j].id){
                            if(selectedMode != dbDataArr[j].payment_mode){
                                ...
                                if(CheckstockistArr == 'add_stockist'){
                                    let obj = {id: dbDataArr[j].id, sku_id: dbDataArr[j].sku_id, payMode: selectedMode, netDiscPercent: netDiscPercent};
                                    selectedModeArr.push(obj);
                                }
                            }
       ...
       $("#zero_config").DataTable(
        ...
        {  
            ...
            [
                ...
                { data: function(row) {//3
                    var result = row.item_code+`<input type="hidden" class="item_code" value="${row.item_code}">`
                    return result;
                  }
                },
                ...
                { data: 'brand_name' },//5
                { data: 'pack' },//6
                { data: 'applicable_gst' },//7
                { data: 'last_year_percent',//8
                ...
                { data: 'type'},//16
                { data: 'hsn_code'},//17
                { data: function(row) {//18
                        var composition = row.composition;
                        return ucwords(composition);;
                    }
                },
                { data: 'stockist_name'},//19
                { data: function(row) {//20
                ...
                { 
                  data: function(row) {//22
                    /*var result = parseFloat(row.net_discount_percent).toFixed(2)
                    return result;*/
                    var netDiscountRateToStockist;
                    let inputMargin = (Disc_margin_item_code[row.item_code])?Disc_margin_item_code[row.item_code] : 10;
                    if(row.payment_mode == 'CN' )
                    {
                        var inputDiscRate = row.discount_percent;
                        netDiscountRateToStockist = parseFloat(inputDiscRate).toFixed(2);
                    }
                    else
                    {
                        var ptr = row.ptr
                        ...
                        {
                            var discountamt = ptr - ((ptr * inputDiscRate) / 100);
                            var marginamt = discountamt * inputMargin / 100;
                            var nrv = discountamt - marginamt;
                            netDiscountRateToStockist = (ptr - nrv) / ptr * 100;
                            netDiscountRateToStockist = parseFloat(netDiscountRateToStockist).toFixed(2)
                        }
                    }
                    return netDiscountRateToStockist;
                  }
                },
                { data: 'stockist_id' },//23
            ],
            columnDefs: [
                ...
                {
                    targets: 22, //net_discount_percent
        ...
        if (largeDataset.length === 0) {//added timer to reload the page if cron runs simultaneously 
            setTimeout(function() {
                location.reload();
            }, 5000);
        }
